...
nonisolated func setupSnapshot(_ app: XCUIApplication, waitForAnimations: Bool = true) {
    MainActor.assumeIsolated {
        Snapshot.setupSnapshot(app, waitForAnimations: waitForAnimations)
    }
}

nonisolated func snapshot(_ name: String, waitForLoadingIndicator: Bool) {
    MainActor.assumeIsolated {
...
nonisolated func snapshot(_ name: String, timeWaitingForIdle timeout: TimeInterval = 20) {
    MainActor.assumeIsolated {
...
open class Snapshot: NSObject {
    ...
    open class func snapshot(_ name: String, timeWaitingForIdle timeout: TimeInterval = 20) {
        ...
        do {
            ...
            let langDir = deviceLanguage.isEmpty
                ? screenshotsDir
                : screenshotsDir.appendingPathComponent(deviceLanguage, isDirectory: true)
            try FileManager.default.createDirectory(at: langDir, withIntermediateDirectories: true)
            let path = langDir.appendingPathComponent("\(simulator)-\(name).png")
            try image.pngData()?.write(to: path, options: .atomic)
        } catch {
            NSLog("Problem writing screenshot: \(name) to \(screenshotsDir)/\(deviceLanguage)/\(simulator)-\(name).png")
            NSLog(error.localizedDescription)
        }
    }
